...
info.waveform_label = char(dummy(1:find(dummy==0, 1)));         %read units until NULL string (suggested by Tom Gaudette)
...
info.ed1.units = char(dummy(1:find(dummy==0, 1)));         %read units until NULL string (suggested by Tom Gaudette)
...
dummy=fread(fid,20,'*uint8',byteorder);
info.ed2.units = char(dummy(1:find(dummy==0, 1)));         %read units until NULL string (suggested by Tom Gaudette)
